...
@interface FMRTCircleView ()
@property (nonatomic,assign) CGFloat radiu;
@end
@implementation FMRTCircleView
//初始化
...
-(void)drawRect:(CGRect)rect{
    ...
    CGFloat angle_start = -M_PI_2; //开始时的弧度  －－－－－ 旋转200度
    CGFloat ff = -M_PI_2;  //记录偏转的角度 －－－－－ 旋转200度
...
-(void)drawArcWithCGContextRef:(CGContextRef)ctx
            ...
            andWithAngle_start:(float)angle_start
              andWithAngle_end:(float)angle_end
                  andWithColor:(NSString *)col
                        andInt:(int)n {
    CGContextMoveToPoint(ctx, point.x, point.y);
    UIColor *color = [UIColor colorWithHexString:col];
                 ...
                 angele:(CGFloat)angele
{
    ...
    NSNumber *numebr =_dataArray[n][@"number"];
    
    NSString * OldNumberStr = [NSString stringWithFormat:@"%.2f",[numebr floatValue]];
    NSString * numberStr;
...
//    UIColor * textColor = color;
    ...
    if (numberStartX>=self.frame.size.width/2){//往右边跑
        [numberStr drawAtPoint:CGPointMake(numberStartX-5+5, numberStartY) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13.0],NSForegroundColorAttributeName:[UIColor colorWithHexString:@"#333"]}];
    }else{
        [numberStr drawAtPoint:CGPointMake(numberStartX+5-5, numberStartY) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13.0],NSForegroundColorAttributeName:[UIColor colorWithHexString:@"#333"]}];
    }
    //指引线下面的text
    ...
    if (textStartX>=self.frame.size.width/2) {//往右边跑
        [_dataArray[n][@"name"] drawInRect:CGRectMake(textStartX-1, textStartY, lineWidth, 50) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12.0],NSForegroundColorAttributeName:[UIColor colorWithHexString:@"#333"],NSParagraphStyleAttributeName:paragraph}];
    }else{
        [_dataArray[n][@"name"] drawInRect:CGRectMake(textStartX+1, textStartY, lineWidth, 50) withAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12.0],NSForegroundColorAttributeName:[UIColor colorWithHexString:@"#333"],NSParagraphStyleAttributeName:paragraph}];
    }
}
