...
{
    ShareViewController *webView=[[ShareViewController alloc]initWithTitle:@"用户服务协议" AndWithShareUrl:loginAndRegisterAndFindPasswordURL];
...
-(void)userNextBtnClick
{
    ...
    //KUserFatelcodeURL
    NSString * url = [NSString stringWithFormat:@"https://www.rongtuojinrong.com/Rongtuoxinsoc/%@",KUserFatelcodeURL] ;
    int timestamp = [[NSDate date] timeIntervalSince1970];
    NSString *token;
    if ([CurrentUserInformation sharedCurrentUserInfo].userId.length > 0) {
       token = EncryptPassword([NSString stringWithFormat:@"AppId=huiyuan&UserId=%@&AppTime=%d",[CurrentUserInformation sharedCurrentUserInfo].userId,timestamp]);
    }else
    {
        token = EncryptPassword([NSString stringWithFormat:@"AppId=huiyuan&UserId=%@&AppTime=%d",self.phoneField.text,timestamp]);
    }
   
    
    NSString *tokenlow=[token lowercaseString];

    NSDictionary * parameter = @{@"UserId":self.phoneField.text,
                                 @"AppId":@"huiyuan",
                                 @"AppTime":[NSNumber numberWithInt:timestamp],
                                 @"Token":tokenlow,
                                 @"tel": self.phoneField.text,
                                 @"leixing":[NSNumber numberWithInteger:self.initOperationStyle]
                                 };
    [FMHTTPClient postPath:url parameters:parameter completion:^(WebAPIResponse *response) {
        [MBProgressHUD hideHUDForView:weakSelf.view animated:YES];
        if (response.code==WebAPIResponseCodeSuccess) {
            
            CodeController *viewController=[[CodeController alloc]initWithUserOperationStyle:UserFindPasswordCodeStyle WithPhoneNumber:self.phoneField.text];
        ...
        else if(response.code==WebAPIResponseCodeFailed)
        {
            ...
            ShowAutoHideMBProgressHUD(weakSelf.view,msg);
        }else
        {
            ShowAutoHideMBProgressHUD(weakSelf.view,@"您的操作太过频繁");
        }
