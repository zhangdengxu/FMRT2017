...
//    __weak __typeof(&*self)weakSelf = self;
    ...
    if (enterpriseVersion.length > 1) {
        //调用企业版
         ...
         [FMHTTPClient getUserRegisterNewWithUserWithUserName:self.nameField.text WithPassword:self.passwordField.text WhithLeixing:@"1" PersonalPhoneNumber:self.telStr imei:openUDIDString withRecommendPeople:kRecommendPersonNamePhoneNumber recommendUserID:kRecommendUserID completion:^(WebAPIResponse *response){
            dispatch_async(dispatch_get_main_queue(), ^(void){
                ...
                if (response.code==WebAPIResponseCodeSuccess) {
                    ...
                    //NSString *url=[NSString stringWithFormat:@"%@?user_id=%@",realNameAuthenKaihuURL,userId];
