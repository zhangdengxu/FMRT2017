...
@property (nonatomic,weak)  LGTextFieldView    *passwordField;
@property (nonatomic,weak)  LGTextFieldView    *sPasswordField;
...
- (void)initWithRegisterControllerFrame{
    ...
    ////文本框
    LGTextFieldView *passwordText= [[LGTextFieldView alloc] initWithFrame:CGRectMake(60.0f, 0, KProjectScreenWidth-30-60, 45.0f)];
    ...
    ////文本框
    LGTextFieldView *spasswordText= [[LGTextFieldView alloc] initWithFrame:CGRectMake(60.0f, 48.5f, KProjectScreenWidth-30-60, 45.0f)];
...
{
    // 输入密码后提交
    NSString * strPassword1 = [self.passwordField.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
    NSString * strPassword2 = [self.sPasswordField.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
    
    if (IsStringEmptyOrNull(strPassword1)) {
        ShowImportErrorAlertView(@"密码不能为空");
        return;
    }
    if ([strPassword1 length]<6||[strPassword1 length]>16) {
        ShowImportErrorAlertView(@"密码长度不符");
        return;
    }
    if (IsStringEmptyOrNull(strPassword2)) {
    ...
    AddObjectForKeyIntoDictionary(strPassword1, @"mima", parameters);
