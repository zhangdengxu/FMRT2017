...
@interface FMRefundMoneyViewController ()<UITableViewDelegate,UITableViewDataSource,LTSCalendarEventSource,HooDatePickerDelegate>{
...
@property (nonatomic, strong) FMMonthDateReFoundMoneyModel * dateModelMonth;
@property (nonatomic, strong) NSMutableArray * dateMonthDaySource;
@property (nonatomic, strong) HooDatePicker *datePicker;
@end

@implementation FMRefundMoneyViewController
...
-(void)headerViewCalenderButtonOnClick:(NSInteger)index
{
    if (index == 1) {
        ...
        if (self.currentMonth.length > 0) {
            NSDate * date = [NSDate retNSStringToNSdateWithYYYY_MM_Nianyue:self.currentMonth];
...
-(void)createCalenderView
{
    ...
    [self calendarDidDateSelected:calendar date:[NSDate date]];
...
-(void)getDateSourceFromNetWork:(NSString *)currentMonth withLastOrRight:(NSInteger)ordict
{
    ...
    NSDictionary * parameter = @{@"UserId":[CurrentUserInformation sharedCurrentUserInfo].userId,
                                 ...
                                 @"Token":tokenlow,
                                 @"Month":[self deleteCurrentMonth:currentMonth]};
    NSString * stringHtml = kXZUniversalTestUrl(@"MonthBackMoneyPlan");// @"https://www.rongtuojinrong.com/esb?CmdId=";
    ...
    [FMHTTPClient postPath:stringHtml parameters:parameter completion:^(WebAPIResponse *response) {
        ...
        if (response.code == WebAPIResponseCodeSuccess) {
            ...
            if (!([dict isMemberOfClass:[NSArray class]]||[dict isMemberOfClass:[NSNull class]])) {
                ...
                [self retContinData:dict[@"Plans"] WithCurrentMonth:month ];
                ...
                self.dateModelMonth = [[FMMonthDateReFoundMoneyModel alloc]init];
                NSString * string1 = dict[@"OuterTotalAmt"];
                NSString * string2 = dict[@"InsideTotalAmt"];
                self.dateModelMonth.outerTotalAmt = string1;
                self.dateModelMonth.insideTotalAmt = string2;
...
-(void )retContinData:(NSMutableArray *)array WithCurrentMonth:(NSString *)month
{
    ...
    for (NSDictionary  * dict in array) {
        ...
        NSString * dayString = dict[@"Day"];
        
        if (![dayString isMemberOfClass:[NSNull class]]) {
            int dayInt = [dayString intValue];
            NSString * key = [NSString stringWithFormat:@"%@-%02d",month,dayInt];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (self.dateModelMonth) {
        ...
        if (indexPath.row == 0) {
            ...
            cell.isRecommendType = 0;
            
            cell.reFoundMoney = self.dateModelMonth;
            if (!self.dateModelMonth.isDay) {
        ...
        }else if(indexPath.row != 0 && indexPath.row < self.dateMonthDaySource.count + 1)
        {
            if (self.dateModelMonth.isDay) {
                ...
                cell.reFoundMoney = self.dateMonthDaySource[indexPath.row - 1];
                return cell;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.row != 0 && indexPath.row < self.dateMonthDaySource.count + 1)
    {
        if (self.dateModelMonth.isDay)
        {
           FMRefoundMoneyListModel * refound = self.dateMonthDaySource[indexPath.row - 1];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    }else if(indexPath.row != 0 && indexPath.row < self.dateMonthDaySource.count + 1)
    {
        if (self.dateModelMonth.isDay) {
            ...
            FMRefoundMoneyListModel * model = self.dateMonthDaySource[indexPath.row - 1];
            CGSize sizeModel ;
            
            sizeModel =  [NSString getStringCGSizeWithMaxSize:CGSizeMake(widthCell, MAXFLOAT) WithFont:[UIFont systemFontOfSize:KCellFontDefault] WithString:model.projName];
...
- (void)calendarDidDateSelected:(LTSCalendarManager *)calendar date:(NSDate *)date
{
    ...
    }else
    {
        ...
        self.dateModelMonth.outerTotalAmt = @"0.00";
        self.dateModelMonth.insideTotalAmt = @"0.00";
...
-(void)getDayDataSourceFromNetWorkWithNSString:(NSString *)key
{
    ...
    NSDictionary * parameter = @{@"UserId":[CurrentUserInformation sharedCurrentUserInfo].userId,
                                 ...
                                 @"AppTime":[NSNumber numberWithInt:timestamp],
                                 @"Token":tokenlow,
                                 @"SearchDate":deleteString};
    NSString * stringHtml = kXZUniversalTestUrl(@"DayBackMoneyPlan");// @"https://www.rongtuojinrong.com/esb?CmdId=";
    ...
    [FMHTTPClient postPath:stringHtml parameters:parameter completion:^(WebAPIResponse *response) {
        ...
        if (response.code == WebAPIResponseCodeSuccess) {
            ...
            if (!([dict isMemberOfClass:[NSArray class]]||[dict isMemberOfClass:[NSNull class]])) {
                ...
                NSArray * listArray = dict[@"Records"];
                if (![listArray isMemberOfClass: [NSNull class]]) {
                    [self.dateMonthDaySource removeAllObjects];

                    for (NSDictionary * dict in listArray) {
                        
                        FMRefoundMoneyListModel * dateModel = [[FMRefoundMoneyListModel alloc]init];
                ...
                self.dateModelMonth = [[FMMonthDateReFoundMoneyModel alloc]init];
                self.dateModelMonth.outerTotalAmt = dict[@"OuterTotalAmt"];
                self.dateModelMonth.insideTotalAmt = dict[@"InsideTotalAmt"];
                self.dateModelMonth.isDay = YES;
