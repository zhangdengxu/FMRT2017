...
@property (nonatomic, weak) UITapGestureRecognizer * twiceTap;
...
-(UIButton *)startButton
{
    if (!_startButton) {
        ...
        [_startButton setImage:[UIImage imageNamed:@"融米学堂_播放-icon_1702"] forState:UIControlStateNormal];
        [_startButton addTarget:self action:@selector(startButtonOnClick:) forControlEvents:UIControlEventTouchUpInside];
...
-(UIImageView *)topVideoButton
{
    if (!_topVideoButton) {
        _topVideoButton = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, KProjectScreenWidth, *_videoHeight)];
        //_topVideoButton.backgroundColor = [UIColor redColor];
    }
    return _topVideoButton;
}
-(void)startButtonOnClick:(UIButton *)button
{
    [self playOrPause];
    [button removeFromSuperview];
    [self.topVideoButton removeFromSuperview];
    
}
- (void)avPlayerButtonOnClick:(UITapGestureRecognizer *)sender {
    
}

#pragma mark - 实例化方法
+(LRLAVPlayerView *)avplayerViewWithVideoUrlStr:(NSString *)urlStr andInitialHeight:(float)height WithWidth:(float)width andSuperView:(UIView *)superView withDefaultVideoUrl:(NSString *)videoUrl{
    static float videoHeight = 0.0;
    static float videoWidth = 0.0;

    
    videoHeight = height;
    videoWidth = width;
    LRLAVPlayerView * view = [[NSBundle mainBundle] loadNibNamed:@"LRLAVPlayerView" owner:nil options:nil].lastObject;
    view.currentOrientation = UIDeviceOrientationPortrait;
    view.videoUrlStr = urlStr;
    view->_videoHeight = &videoHeight;
    view->_videoWidth = &videoWidth;
    view.avplayerSuperView = superView;
    view.userInteractionEnabled = YES;
    
    [view addSubview:view.topVideoButton];
    [view bringSubviewToFront:view.topVideoButton];
    [view.topVideoButton sd_setImageWithURL:[NSURL URLWithString:videoUrl] placeholderImage:[UIImage imageNamed:@""]];
    
    
    [view addSubview:view.startButton];
    [view bringSubviewToFront:view.startButton];

    [view.startButton makeConstraints:^(MASConstraintMaker *make) {
...
#pragma mark - 判断是否处理视频屏幕旋转
-(BOOL)goOrientationChangeWithisPlay
{
    if (_isFullScreen) {
        return YES;
    }else
...
-(void)orientationChanged:(NSNotification *)notification{
    if (![self goOrientationChangeWithisPlay]) {
        return;
    }
...
-(void)toOrientation:(UIInterfaceOrientation)orientation{
    ...
    UIInterfaceOrientation currentOrientation = self.currentOrientation;
    ...
    //UIInterfaceOrientation orientationlinshi = [UIApplication sharedApplication].statusBarOrientation;
    
    [UIView beginAnimations:nil context:nil];
    //旋转视频播放的view和显示亮度的view
    self.transform = [self getOrientationWithInteger:orientation];
    //self.lightView.transform = [self getOrientationWithInteger:orientation];
    [UIView setAnimationDuration:0.5];
    [UIView commitAnimations];
}

//根据状态条旋转的方向来旋转 avplayerView
-(CGAffineTransform)getOrientationWithInteger:(UIInterfaceOrientation )orientation{
...
-(void)iniiitWithFrame:(CGRect)frame{
    NSAssert(NO, @"请不要使用此实例化方法");
//    return nil;
}
