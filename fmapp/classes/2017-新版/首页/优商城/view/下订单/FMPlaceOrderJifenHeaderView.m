...
#import "FMPlaceOrderJifenHeaderView.h"
#import "SDCycleScrollView.h"

#import "FMShopSpecModel.h"


@interface FMPlaceOrderJifenHeaderView ()<SDCycleScrollViewDelegate,UIScrollViewDelegate>
...
@implementation FMPlaceOrderJifenHeaderView
...
//        _oldPrice.textColor = [HXColor colorWithHexString:@"#aaa"];
        ...
        _oldPrice.frame = CGRectMake(12, CGRectGetMaxY(self.moneyTitle.frame) , 90, KDefauletCellItemLittleHeigh);
    }
    return _oldPrice;
}
...
-(UILabel *)activityLabel
{
    if (!_activityLabel) {
        ...
        _activityLabel.frame = CGRectMake(12, CGRectGetMaxY(self.oldPrice.frame) + 3 , KProjectScreenWidth - 24, KDefauletCellItemLittleHeigh);
    }
...
-(void)changeCreateUIScrollView
{
    ...
    {
        self.functionView.frame = CGRectMake(0,CGRectGetMaxY(self.oldPrice.frame) + KMargionItem + 1,KProjectScreenWidth,KDefauletCellItemLargeHeigh);
    }
...
{
    CGFloat heigh = view2.frame.origin.y + KMargionItem + view2.frame.size.height - view1.frame.origin.y + KMargionItem ;
    if (KProjectScreenWidth == 320) {
        self.titleBackageView.frame = CGRectMake(0,view1.frame.origin.y - KMargionItem - KMargionItem , KProjectScreenWidth, heigh + KMargionItem);
        
    }else
    {
        self.titleBackageView.frame = CGRectMake(0,view1.frame.origin.y - KMargionItem , KProjectScreenWidth, heigh);
...
-(void)setShopDetailModel:(FMSelectShopInfoModel *)shopDetailModel
{
    ...
    if (fulljifen_ex.length > 0 && [fulljifen_ex integerValue] != 0) {
        ...
        self.scoreLabel.frame = CGRectMake(12, scoreRect.origin.y, scoreRect.size.width, scoreRect.size.height);
    ...
    }else
    {
        ...
        if (shopDetailModel.mktPrice) {
            // 给数字划线
            NSString *oldPrice = [NSString stringWithFormat:@"%f",[ shopDetailModel.mktPrice floatValue]];
            NSUInteger length = [[NSString stringWithFormat:@"价格￥%@",oldPrice] length];
            NSMutableAttributedString *attri = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"价格￥%@",oldPrice]];
            [attri addAttribute:NSStrikethroughStyleAttributeName value:@(NSUnderlinePatternSolid | NSUnderlineStyleThick) range:NSMakeRange(3, length - 3)];
    ...
    [self.checkShopPriceDetail setTitle:@"查看会员价" forState:UIControlStateNormal];
    ...
    {
         [self changeBackGroundWithFirstView:self.shopTitle withSecondView:self.oldPrice];
    }
    ...
    if (shopDetailModel.isAllShopInfo) {
        ...
        if (strUrl.length <= 1) {
             self.funcSelectTitle.text = @"请选择商品样式";
        }else
        {
