...
#import "XZConfirmPaymentHeader.h"
#import "XZShoppingOrderAddressModel.h"
#import "FMShopSpecModel.h"

@interface XZConfirmPaymentHeader ()
/** 有收货人地址 */
...
@implementation XZConfirmPaymentHeader
...
- (void)setUpConfirmPaymentHeader {
    ...
    [labelProfmpt mas_updateConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self);
        make.right.equalTo(self);
        make.top.equalTo(self);
        make.height.equalTo(@25);
    }];
    labelProfmpt.textColor = XZColor(255, 102, 51);
    labelProfmpt.textAlignment = NSTextAlignmentCenter;
    labelProfmpt.text = @"请您在提交订单后10分钟内完成支付，否则订单会自动取消！";
    if (KProjectScreenWidth > 350) {
        labelProfmpt.font = [UIFont systemFontOfSize:13];
    }else {
        labelProfmpt.font = [UIFont systemFontOfSize:11];
    }
    labelProfmpt.backgroundColor = XZColor(254, 255, 243);
    ...
    [line mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self);
        make.right.equalTo(self);
        make.top.equalTo(labelProfmpt.mas_bottom);
        make.height.equalTo(@0.5);
    }];
    line.backgroundColor = KDefaultOrBackgroundColor;
    
    // 添加有收货人地址的view
    [self headerViewWithUserNameAndNumber:line];
    
    // 添加有收货人地址的view
    [self headerViewNoDefaultAddress:line];
    
}

/** 有收货人地址 */
- (void)headerViewWithUserNameAndNumber:(UILabel *)line {
...
/** 请选择地址 */
- (void)headerViewNoDefaultAddress:(UILabel *)line {
    // WithFrame:CGRectMake(0, 0, KProjectScreenWidth, 86)
    ...
    [addNewAdress mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(tableHeaderNoAddress.mas_centerY);
        make.centerX.equalTo(tableHeaderNoAddress.mas_centerX);
    }];
