//
//  YYAlreadyAnnounceResultsCell.m
//  fmapp
//
...
#import "Fm_Tools.h"
@interface YYAlreadyAnnounceResultsCell ()
/** 夺宝商品名称  */
...
@implementation YYAlreadyAnnounceResultsCell
...
- (void)createContentView{
    ...
    winPrintView.image = [UIImage imageNamed:@"1101恭喜中奖"];
...
-(void)setStatus:(YYAnnounceResultsNewModel *)status{
    ...
    /**  获奖用户 */
//    self.winLabel.text = [NSString stringWithFormat:@"获奖用户:%@",status.win_user];
    NSString *tel = [status.win_user stringByReplacingCharactersInRange:NSMakeRange(3, 4) withString:@"****"];
    self.winLabel.text = [NSString stringWithFormat:@"获得者:%@", tel];
    ...
    if ([status.is_win integerValue] == 0) {
        ...
        self.winPrintView.hidden = YES;
    }else if([status.is_win integerValue] == 1){
        self.winPrintView.image = [UIImage imageNamed:@"1101恭喜中奖奖"];
        self.winPrintView.hidden = NO;
