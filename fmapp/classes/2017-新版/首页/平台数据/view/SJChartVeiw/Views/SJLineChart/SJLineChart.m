...
- (void)setXMarkTitlesAndValues:(NSArray *)xMarkTitlesAndValues titleKey:(NSString *)titleKey valueKey:(NSString *)valueKey descpKey:(NSString *)desKey{
    
    _xMarkTitlesAndValues = xMarkTitlesAndValues;
    
    xMarkTitles = [NSMutableArray array];
    valueArray = [NSMutableArray array];
    despArray = [NSMutableArray array];
    for (NSDictionary *dic in xMarkTitlesAndValues) {
        ...
        if ([dic objectForKey:desKey]) {
            [despArray addObject:[dic objectForKey:desKey]];
        }
...
- (void)mapping {
    ...
    self.chartLineView.despArray = despArray;
...
//    
//    self.scrollView.contentSize = CGSizeMake(self.chartLineView.bounds.size.width + 100, self.chartLineView.bounds.size.height);
}
