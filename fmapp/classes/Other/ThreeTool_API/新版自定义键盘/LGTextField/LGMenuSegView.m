...
-(id)initWithFrame : (CGRect)frame  TitleArray :(NSArray *)titleArray
{
    ...
    if (self) {
        ...
        float btnWidth,origin_x;
        if ([UIScreen mainScreen].bounds.size.width  < 350) {
            btnWidth = 55;
            origin_x = [UIScreen mainScreen].bounds.size.width - btnWidth * titleArray.count;
        ...
        for (int i =0; i<titleArray.count; i++) {
            UIButton *topButton = [[UIButton alloc]initWithFrame:CGRectMake(origin_x + i*btnWidth,0,btnWidth,frame.size.height)];
            topButton.tag = 31+i;
