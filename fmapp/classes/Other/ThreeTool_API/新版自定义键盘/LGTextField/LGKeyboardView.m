...
#define kButtonHeighLightColor [UIColor colorWithRed:241.0/255.0 green:241.0/255.0 blue:241.0/255.0 alpha:1]
...
-(void)addNumberBoard
{
    ...
    }else
    {
        ...
        while (numberKeyboardArray.count>0) {
            ...
            //int random = arc4random()%(numberKeyboardArray.count);
            NSString *title =numberKeyboardArray[index];
            [randomBtnTitle addObject:title];
            [numberKeyboardArray removeObjectAtIndex:index];
    ...
    for (int i=0; i<12; i++) {
        ...
        [keyBoardBtn setBackgroundImage:[self retUIImageWithColor:[UIColor whiteColor]] forState:UIControlStateNormal];
        [keyBoardBtn setBackgroundImage:[self retUIImageWithColor:kButtonHeighLightColor] forState:UIControlStateHighlighted];
...
-(UIImage *)retUIImageWithColor:(UIColor *)color
{
...
-(void)addEnglishKeyboard :(NSString *)type
{
    ...
    if ([type isEqualToString:@"ABC"]) {
        titleArray = [NSMutableArray arrayWithArray:@[@"Q",@"W",@"E",@"R",@"T",@"Y",@"U",@"I",@"O",@"P",@"A",@"S",@"D",@"F",@"G",@"H",@"J",@"K",@"L",@"Z",@"X",@"C",@"V",@"B",@"N",@"M"]];
        if (self.isRandom) {
        ...
        }else
        {
            NSInteger index = 0;
            while (titleArray.count>0) {
                [newArr addObject:titleArray[index]];
    ...
    //第四列的按钮
    XZButton *defaultBtn = [XZButton buttonWithType:UIButtonTypeCustom];
    defaultBtn.buttonsType = XZButtonTypePicTitleDistance;
    ...
    [defaultBtn setTitle:@"融托金融安全键盘" forState:UIControlStateNormal];
    [defaultBtn setImage:[UIImage imageNamed:@"我的-登陆-键盘-icon"] forState:UIControlStateNormal];
