...
- (MXPullDownMenu *)initWithArray:(NSArray *)array selectedColor:(UIColor *)color WithArray:(NSArray *)titleArr
{
    self = [super init];
    if (self) {
        
        self.frame = CGRectMake(0, 0, KProjectScreenWidth, 36);
        ...
        for (int i = 0; i < _numOfMenu; i++) {
            ...
            CATextLayer *title = [self creatTextLayerWithNSString:titleArr[i] withColor:_menuColor andPosition:position];
            [self.layer addSublayer:title];
            
            if (i == (_numOfMenu - 1)) {
                self.lastCaTextLayer = title;
            }
            
            NSArray *arr=[array objectAtIndex:i];
            CATextLayer *title1 = [self creatTextLayerWithNSString:arr[0] withColor:_menuColor andPosition:position];
            ...
            if (i != _numOfMenu - 1) {
                ...
                CAShapeLayer *separator = [self creatSeparatorLineWithColor:KSepLineColorSetup andPosition:separatorPosition];
...
-(void)didSelectThisItem:(NSInteger)indexRow WithColumn:(NSInteger)column
{
    _currentSelectedMenudIndex = column;
    NSIndexPath * indexPath = [NSIndexPath indexPathForRow:indexRow inSection:column];
