...
- (void)createMainView
{
    ...
    if (!IsStringEmptyOrNull([[NSUserDefaults standardUserDefaults] stringForKey:@"userLogo"])) {
       ...
       NSString *Url=[[NSUserDefaults standardUserDefaults] stringForKey:@"userLogo"];
        logoUrl=[NSString stringWithFormat:@"http://%@",Url];
    }
    else
    {
        logoUrl=@"https://www.rongtuojinrong.com/Tpl/Home/Static/images/finance_10.png";
    }
...
//    [logo setImageWithURL:[NSURL URLWithString:logoUrl] placeholderImage:createImageWithColor(KUIImageViewDefaultColor)];
    
    [logo sd_setImageWithURL:[NSURL URLWithString:logoUrl] placeholderImage:createImageWithColor(KUIImageViewDefaultColor)];
    self.logoImage=logo;
...
- (void)createPassword:(NSString*)string{
    ...
    if (string.length < 4) {
        [self setErrorTip:@"密码长度需大于4位" errorPswd:string];
        return;
    }
    // 输入密码
...
- (void)animateUnlock{
    ...
    static NSTimeInterval duration = 0.6;
