...
static CGFloat const kHooDatePickerLineWidth = 1.0f;
...
#define kHooDatePickerHighlightColor  [UIColor lightGrayColor]
...
#define kHooDatePickerFontColorLabelSelected [UIColor blackColor]
...
- (void)highlightLabelInArray:(NSMutableArray*)labels atIndex:(NSInteger)index {
    ...
    for (int i = 0; i < labels.count; i++) {
        ...
        if (i != index) {
            ...
            label.textColor = kHooDatePickerFontColorLabelSelected;//self.highlightColor;
...
- (NSTimeZone *)timeZone {
    if (!_timeZone) {
        _timeZone = [NSTimeZone timeZoneWithAbbreviation:@"CCD"];
    }
    return _timeZone;
}
