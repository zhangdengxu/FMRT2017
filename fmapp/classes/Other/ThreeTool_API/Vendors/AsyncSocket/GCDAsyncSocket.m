...
- (void)doWriteData
{
	...
	if (error)
	{
        if (delegateQueue && [delegate respondsToSelector:@selector(socket:didWriteDataFailed:tag:)])
        {
            __strong id theDelegate = delegate;
            long theWriteTag = currentWrite->tag;
            NSData* theWriteData = currentWrite->buffer;
            dispatch_async(delegateQueue, ^{ @autoreleasepool {
                
                [theDelegate socket:self
                 didWriteDataFailed:theWriteData
                                tag:theWriteTag];
            }});
        }
...
- (void)completeCurrentWrite
{
    ...
    if (delegateQueue && [delegate respondsToSelector:@selector(socket:didWriteData:WithTag:)])
	{
		__strong id theDelegate = delegate;
		long theWriteTag = currentWrite->tag;
        NSData* theWriteData = currentWrite->buffer;
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
            ...
            [theDelegate socket:self didWriteData:theWriteData WithTag:theWriteTag];
		}});
	}
