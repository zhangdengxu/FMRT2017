...
-(instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _redius = (CGRectGetHeight(self.frame) -60*k_Width_Scale)/4 + 8;
        if (KProjectScreenWidth <375) {
            _redius  = (CGRectGetHeight(self.frame) -60*k_Width_Scale)/4 + 10;
        }
        _ringWidth = 40;
    }
    return self;
...
- (void)setValueDescriptionArr:(NSArray *)valueDescriptionArr{
    _valueDescriptionArr = valueDescriptionArr;
}
...
//        NSLog(@"%f",currentSpace);
        ...
        CGPoint endx = CGPointMake(self.chartOrigin.x + sin(midSpace) * longLen + 3, self.chartOrigin.y - cos(midSpace)*longLen + 3);
...
//        NSLog(@"%@%@",NSStringFromCGPoint(begin),NSStringFromCGPoint(endx));
        ...
        [self drawLineWithContext:contex andStarPoint:begin andEndPoint:endx andIsDottedLine:NO andColor:[UIColor blackColor]];
        
        NSString *description = self.valueDescriptionArr[i];
        
        CGPoint secondP = CGPointZero;
        
        CGSize size = [[NSString stringWithFormat:@"%@\n%.02f%c",description,[obj floatValue] / _totolCount * 100,'%'] boundingRectWithSize:CGSizeMake(200, 100) options:NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12*k_Width_Scale]} context:nil].size;
        
        NSInteger sizefont =0;
        if (KProjectScreenWidth <375) {
            sizefont = 11;
        }else{
            sizefont = 12;
...
//          [self drawText:[NSString stringWithFormat:@"%@\n%.02f%c",description,[obj floatValue] / _totolCount * 100,'%'] andContext:contex atPoint:CGPointMake(secondP.x + 3, secondP.y - size.height / 2) WithColor:[UIColor colorWithHexString:@"#333333"] andFontSize:sizefont*k_Width_Scale];

            UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(secondP.x + 3, secondP.y - size.height - 8, 65, 60)];
            label.numberOfLines = 0;
            label.textColor = [UIColor colorWithHexString:@"#333333"];
            label.font = [UIFont systemFontOfSize:12];
            label.text = [NSString stringWithFormat:@"%@\n%.02f%c",description,[obj floatValue] / _totolCount * 100,'%'] ;
            [self addSubview:label];
            
            
        }else{
             secondP =CGPointMake(endx.x - 20*k_Width_Scale, endx.y);
//            [self drawText:[NSString stringWithFormat:@"%@\n%.02f%c",description,[obj floatValue] / _totolCount * 100,'%'] andContext:contex atPoint:CGPointMake(secondP.x - size.width - 3, secondP.y - size.height/2) WithColor:[UIColor colorWithHexString:@"#333333"] andFontSize:sizefont*k_Width_Scale];
            
            UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(secondP.x - size.width - 3, secondP.y - size.height-10, 65, 60)];
            label.numberOfLines = 0;
            label.textColor = [UIColor colorWithHexString:@"#333333"];
            label.font = [UIFont systemFontOfSize:12];
            label.text = [NSString stringWithFormat:@"%@\n%.02f%c",description,[obj floatValue] / _totolCount * 100,'%'] ;
          ...
          [self drawLineWithContext:contex andStarPoint:endx andEndPoint:secondP andIsDottedLine:NO andColor:[UIColor blackColor]];
//        [self drawPointWithRedius:3*k_Width_Scale andColor:[UIColor blackColor] andPoint:secondP andContext:contex];
