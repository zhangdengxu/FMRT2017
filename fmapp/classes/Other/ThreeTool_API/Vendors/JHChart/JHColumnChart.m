...
//        _maxHeight += 4;
    ...
    if (_xShowInfoText.count == _valueArr.count&&_xShowInfoText.count>0) {
        ...
        for (NSInteger i = 0; i<_xShowInfoText.count; i++) {
            ...
            CGFloat wid =  count * _columnWidth + 20;
            ...
            textLayer.frame = CGRectMake( i * (count * _columnWidth + _typeSpace) + _typeSpace + _originSize.x - 10, CGRectGetHeight(self.frame) - _originSize.y+5,wid, size.height);
    ...
    for (NSInteger i = 0; i<_valueArr.count; i++) {
        ...
        for (NSInteger j = 0; j<arr.count; j++) {
            ...
            itemsView.backgroundColor = (UIColor *)(_columnBGcolorsArr.count<arr.count?[UIColor colorWithHexString:@"86c6ec"]:_columnBGcolorsArr[j]);
            [UIView animateWithDuration:1 animations:^{
                ...
                if (finished) {
                    ...
                    CGSize size = [str boundingRectWithSize:CGSizeMake(_columnWidth, MAXFLOAT) options:NSStringDrawingUsesFontLeading|NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingTruncatesLastVisibleLine attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12]} context:nil].size;
                    
                    textLayer.frame = CGRectMake((i * arr.count + j)*_columnWidth + i*_typeSpace+_originSize.x + _typeSpace, CGRectGetHeight(self.frame) - height - _originSize.y -3 - size.height, _columnWidth + 5, size.height);
                    ...
                    textLayer.alignmentMode = kCAAlignmentCenter;
                    textLayer.contentsScale = [UIScreen mainScreen].scale;
                    textLayer.foregroundColor = [UIColor colorWithHexString:@"#333333"].CGColor;
