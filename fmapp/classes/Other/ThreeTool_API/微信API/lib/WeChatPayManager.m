...
-(NSString *)sendPrepay:(NSMutableDictionary *)prePayParams
{
    ...
    if ( [return_code isEqualToString:@"SUCCESS"] )
    {
        ...
        if( [sign isEqualToString:send_sign]){
            ...
            {
                NSString * err_code_des = [resParams objectForKey:@"err_code_des"];
                if ([err_code_des isEqualToString:@"商户订单号重复"]) {
                    NSLog(@"商户订单号重复");
                    self.lastErrCode = 1000;
                    [self.debugInfo appendFormat:@"商户订单号重复"];
                }
            }
        }else{
            self.lastErrCode = 1;
...
//    NSString* device = [[UIDevice currentDevice] identifierForVendor].UUIDString;
    ...
    NSNumber *pay_type_Shop = [orderDic objectForKey:@"pay_type_Shop"];
#pragma mark --- 警告：调整价格，以分为单位
//    NSString * price = [NSString stringWithFormat:@"%.0f",[[orderDic objectForKey:@"money"] doubleValue]];
    NSString * price = orderDic[@"money"];
    ...
    //支付设备号或门店号
//    NSString* orderDevice = device;//作废
    NSString* orderDevice = @"WEB";//作废
    //支付类型，固定为APP
    ...
    if ([pay_type_Shop integerValue] == 11) {
        
        
        [packageParams setObject:[orderDic objectForKey:@"time_start"] forKey:@"time_start"];
        [packageParams setObject:[orderDic objectForKey:@"time_expire"] forKey:@"time_expire"];
    ...
    Log(@"========%@ %@ %@============",self.appId,self.mchId,prePayid);
