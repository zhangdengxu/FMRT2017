...
Boolean IsNormalMobileNum(NSString  *userMobileNum){
    ...
    NSString *patternStr = [NSString stringWithFormat:@"^(0?1[1345678]\\d{9})$|^((0(10|2[1-3]|[3-9]\\d{2}))?[1-9]\\d{6,7})$"];
...
void ShowFMSoppingMBProgressHUD(UIView *onView, NSString *labelText)
{
    if (!onView || [labelText length] <= 0)
        return;
    ...
    hud.yOffset = KProjectScreenHeight/2 - 105;
    hud.cornerRadius = 0;
    hud.removeFromSuperViewOnHide = YES;
    hud.labelText = labelText;
